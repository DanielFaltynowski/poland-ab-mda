...
dane_wykresy %>%
  ...
  theme_minimal() +
  labs(
    title = "Silna asymetria prawostronna (Skośność > 1.2)",
    x = "Wartość", y = "Liczba wystąpień"
  ) +
  theme(strip.text = element_text(size = 11, face = "bold"))
...
# ==============================================================================
# TOP 25 NAJSILNIEJSZYCH KORELACJI
# ==============================================================================
...
cor_matrix_upper %>%
  ...
  # ZAMIANA: zamiast filter, wybieramy 25 najwyższych wartości abs_value
  slice_max(abs_value, n = 25) %>% 
  arrange(abs_value) %>% # Porządkowanie dla lepszego wykresu
  ggplot(aes(x = reorder(paste(Var1, "-", Var2), abs_value), 
             y = value, 
             fill = value > 0)) +
  geom_col(show.legend = FALSE) + 
  geom_text(aes(label = round(value, 2)), 
            hjust = ifelse(cor_matrix_upper %>% 
                             reshape2::melt(na.rm = TRUE) %>% 
                             mutate(abs_value = abs(value)) %>% 
                             slice_max(abs_value, n = 25) %>% 
                             pull(value) > 0, -0.2, 1.2), 
            size = 3.5, 
            fontface = "bold") +
  coord_flip() +
  ...
  theme_minimal() +
  labs(
    title = "Top 25 najsilniejszych korelacji",
    subtitle = "Ranking według wartości bezwzględnej",
    x = "Para zmiennych",
...
# ==============================================================================
# TOP 25 KORELACJI - POPRAWIONY BŁĄD 'value'
# ==============================================================================
...
# Przygotowanie danych do wykresu w osobnej ramce, by uniknąć błędów w ggplot
plot_data <- cor_matrix_upper %>%
  reshape2::melt(na.rm = TRUE) %>%
  mutate(abs_value = abs(value)) %>%
  slice_max(abs_value, n = 25) %>%
  mutate(is_positive = value > 0)

ggplot(plot_data, aes(x = reorder(paste(Var1, "-", Var2), abs_value), 
                      y = value, 
                      fill = is_positive)) +
  geom_col(show.legend = FALSE) + 
  geom_text(aes(label = round(value, 2), 
                # Jeśli value > 0, przesuń w prawo (-0.2), jeśli ujemne, przesuń w lewo (1.2)
  ...
  labs(
    title = "Top 25 najsilniejszych korelacji",
    subtitle = "Niebieski: dodatnia (+), Czerwony: ujemna (-)",
    x = "Para zmiennych",
    y = "Współczynnik korelacji"
  ) +
  theme(
...
top_25_pairs <- cor_matrix_upper %>%
  reshape2::melt(na.rm = TRUE) %>%
  mutate(abs_value = abs(value)) %>%
  slice_max(abs_value, n = 25)

# 2. Przygotowanie danych do facetowania (map_df połączy wszystkie pary)
plot_data_scatter <- top_25_pairs %>%
  rowwise() %>%
  do({
    # Wyciągamy surowe dane dla każdej pary
    v1 <- as.character(.$Var1)
    v2 <- as.character(.$Var2)
    data.frame(
      x_val = dane[[v1]],
      y_val = dane[[v2]],
      pair_label = paste0(v1, " vs ", v2, "\n(r = ", round(.$value, 2), ")"),
      is_positive = .$value > 0
    )
  }) %>%
  ungroup()

# 3. Generowanie zbiorczego wykresu
ggplot(plot_data_scatter, aes(x = x_val, y = y_val, color = is_positive)) +
  geom_point(alpha = 0.5, size = 0.8) +
  geom_smooth(method = "lm", color = "black", linewidth = 0.5, se = FALSE) +
  facet_wrap(~ pair_label, scales = "free", ncol = 5) +
  scale_color_manual(values = c("TRUE" = "steelblue", "FALSE" = "firebrick")) +
  theme_minimal() +
  labs(
    title = "Top 25 najsilniejszych korelacji - wykresy rozrzutu",
    subtitle = "Z linią trendu (lm); wolne skale dla każdej pary",
    x = "Wartość zmiennej X",
    y = "Wartość zmiennej Y"
  ) +
  theme(
    legend.position = "none",
    strip.text = element_text(size = 7, face = "bold"), # Mniejszy tekst nagłówków
